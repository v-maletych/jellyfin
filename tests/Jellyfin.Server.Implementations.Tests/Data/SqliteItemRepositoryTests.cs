...
namespace Jellyfin.Server.Implementations.Tests.Data
{
    public class SqliteItemRepositoryTests
    {
        ...
        [Fact]
        public void Map_ProductionLocationsWithEmptyEntries_DoesNotSaveEmptyLocations()
        {
            var item = new Person
            {
                Id = Guid.NewGuid(),
                ProductionLocations = [string.Empty, "New York City, New York, USA", " "]
            };

            var entity = _sqliteItemRepository.Map(item);

            Assert.Equal("New York City, New York, USA", entity.ProductionLocations);
        }
